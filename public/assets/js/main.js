...
function createSoilTriangle() {
  ...
  const triangleHeight = Math.sqrt(3)/2 * (width - margin.left - margin.right);
  const points = [
  ...
  const leftMidpoint = [(points[0][0] + points[1][0])/2, (points[0][1] + points[1][1])/2];
  svg.append("text")
    .attr("x", leftMidpoint[0] - 40)
    .attr("y", leftMidpoint[1])
    .attr("text-anchor", "end")
    .attr("transform", `rotate(-60, ${leftMidpoint[0] - 40}, ${leftMidpoint[1]})`)
    .text("Clay Separate, %");
  
  // Add arrow for Clay (aligned with left side, pointing upward)
  const leftSideAngle = Math.atan2(points[0][1] - points[1][1], points[0][0] - points[1][0]);
  ...
  svg.append("path")
    .attr("d", `M${leftMidpoint[0] - leftArrowLength/2 * Math.cos(leftSideAngle) + leftOffsetX},
                 ${leftMidpoint[1] - leftArrowLength/2 * Math.sin(leftSideAngle) + leftOffsetY} 
                 L${leftMidpoint[0] + leftArrowLength/2 * Math.cos(leftSideAngle) + leftOffsetX},
                 ${leftMidpoint[1] + leftArrowLength/2 * Math.sin(leftSideAngle) + leftOffsetY}`)
    .attr("stroke", "black")
    .attr("marker-end", "url(#arrowhead)");
  
  // Bottom side - Sand percentage (right to left)
  const bottomMidpoint = [(points[1][0] + points[2][0])/2, (points[1][1] + points[2][1])/2];
  svg.append("text")
  ...
  svg.append("path")
    .attr("d", `M${bottomMidpoint[0] + bottomArrowLength/2},${bottomMidpoint[1] + bottomOffset} 
                 L${bottomMidpoint[0] - bottomArrowLength/2},${bottomMidpoint[1] + bottomOffset}`)
    .attr("stroke", "black")
    .attr("marker-end", "url(#arrowhead)");
  
  // Right side - Silt percentage (top to bottom)
  const rightMidpoint = [(points[0][0] + points[2][0])/2, (points[0][1] + points[2][1])/2];
  svg.append("text")
    .attr("x", rightMidpoint[0] + 40)
    .attr("y", rightMidpoint[1])
    .attr("text-anchor", "start")
    .attr("transform", `rotate(60, ${rightMidpoint[0] + 40}, ${rightMidpoint[1]})`)
    .text("Silt Separate, %");
  
  // Add arrow for Silt (aligned with right side, pointing downward)
  const rightSideAngle = Math.atan2(points[2][1] - points[0][1], points[2][0] - points[0][0]);
  ...
  svg.append("path")
    .attr("d", `M${rightMidpoint[0] - rightArrowLength/2 * Math.cos(rightSideAngle) + rightOffsetX},
                 ${rightMidpoint[1] - rightArrowLength/2 * Math.sin(rightSideAngle) + rightOffsetY} 
                 L${rightMidpoint[0] + rightArrowLength/2 * Math.cos(rightSideAngle) + rightOffsetX},
                 ${rightMidpoint[1] + rightArrowLength/2 * Math.sin(rightSideAngle) + rightOffsetY}`)
    .attr("stroke", "black")
    .attr("marker-end", "url(#arrowhead)");
  ...
  function percentToPoint(clay, sand, silt) {
    ...
    const x = points[0][0] * normalizedClay + points[1][0] * normalizedSand + points[2][0] * normalizedSilt;
    const y = points[0][1] * normalizedClay + points[1][1] * normalizedSand + points[2][1] * normalizedSilt;
  ...
  d3.json("./assets/data/data.json").then(function(soilData) {
    ...
    Object.entries(soilData).forEach(([texture, points]) => {
      ...
      const [cx, cy] = percentToPoint(centroid.clay, centroid.sand, centroid.silt);
      
      soilRegions.append("text")
        .attr("x", cx)
        .attr("y", cy)
        .attr("text-anchor", "middle")
        .attr("font-size", "10px")
  ...
  function calculateCentroid(points) {
    ...
    let claySum = 0, sandSum = 0, siltSum = 0;
  ...
  function isPointInPolygon(clay, sand, silt, polygonPoints) {
    ...
    for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
      ...
      const intersect = ((yi > sand) !== (yj > sand)) &&
  ...
  function updateCoordinates(clay, sand, silt) {
    d3.select("#coordinates").text(
      `Clay: ${Math.round(clay)}%, Sand: ${Math.round(sand)}%, Silt: ${Math.round(silt)}%`
    );
    ...
    const densityFactor = parseFloat(document.getElementById("density-input").value);
    
    // Update water characteristics
    updateWaterCharacteristics(Math.round(clay), Math.round(sand), om, densityFactor);
    
    // Update soil texture display
    const texture = getSoilTexture(clay, sand, silt);
    d3.select("#soil-texture-display").text(`Soil Texture: ${texture.toUpperCase()}`);
  }
  ...
  const drag = d3.drag()
    .on("drag", function(event) {
      ...
      const totalArea = Math.abs((points[1][0] - points[0][0]) * (points[2][1] - points[0][1]) - 
                                (points[2][0] - points[0][0]) * (points[1][1] - points[0][1]));
      
      const area1 = Math.abs((points[1][0] - x) * (points[2][1] - y) - 
                            (points[2][0] - x) * (points[1][1] - y));
      const area2 = Math.abs((points[0][0] - x) * (points[2][1] - y) - 
                            (points[2][0] - x) * (points[0][1] - y));
      const area3 = Math.abs((points[0][0] - x) * (points[1][1] - y) - 
  ...
  function setupInputHandlers() {
    ...
    document.getElementById("update-point").addEventListener("click", function() {
      ...
      // Validate inputs
      if (clayValue < 0 || clayValue > 100 || sandValue < 0 || sandValue > 100 || siltValue < 0) {
        alert("Invalid values. Clay and sand must be between 0-100, and their sum must not exceed 100.");
        return;
      }
    ...
    const updateSilt = function() {
      const clayValue = parseFloat(document.getElementById("clay-input").value) || 0;
      const sandValue = parseFloat(document.getElementById("sand-input").value) || 0;
      const siltValue = 100 - clayValue - sandValue;
      
      document.getElementById("silt-input").value = Math.max(0, Math.round(siltValue));
    };
...
function updateWaterCharacteristics(clay, sand, om, densityFactor) {
  // Calculate silt
  const silt = 100 - clay - sand;
  
  // Get water characteristics
  const waterCharacteristics = calculateSoilWaterCharacteristics(sand, clay, om, densityFactor);
  
  // Update display values
  document.getElementById("field-capacity").textContent = waterCharacteristics.fieldCapacity;
  document.getElementById("wilting-point").textContent = waterCharacteristics.wiltingPoint;
  document.getElementById("plant-available-water").textContent = waterCharacteristics.plantAvailableWater;
  document.getElementById("saturation").textContent = waterCharacteristics.saturation;
  document.getElementById("saturated-conductivity").textContent = waterCharacteristics.saturatedConductivity;
  ...
  document.getElementById("field-capacity-bar").style.width = `${Math.min(100, (fcValue / maxFC) * 100)}%`;
  document.getElementById("wilting-point-bar").style.width = `${Math.min(100, (wpValue / maxWP) * 100)}%`;
  document.getElementById("plant-available-water-bar").style.width = `${Math.min(100, (pawValue / maxPAW) * 100)}%`;
  document.getElementById("saturation-bar").style.width = `${Math.min(100, (satValue / maxSat) * 100)}%`;
  document.getElementById("saturated-conductivity-bar").style.width = `${Math.min(100, (ksValue / maxKs) * 100)}%`;
  ...
  const satPosition = containerHeight * (1 - satValue / 100) * 0.9 + containerHeight * 0.1;
  const fcPosition = containerHeight * (1 - fcValue / 100) * 0.9 + containerHeight * 0.1;
  const wpPosition = containerHeight * (1 - wpValue / 100) * 0.9 + containerHeight * 0.1;
  
  // Update positions of water levels
  document.querySelector(".saturation-level").style.top = `${satPosition}px`;
  document.querySelector(".field-capacity-level").style.top = `${fcPosition}px`;
  document.querySelector(".wilting-point-level").style.top = `${wpPosition}px`;
  
  // Update positions of water zones
  document.querySelector(".gravitational-zone").style.top = `${satPosition}px`;
  document.querySelector(".gravitational-zone").style.height = `${fcPosition - satPosition}px`;
  
  document.querySelector(".available-zone").style.top = `${fcPosition}px`;
  document.querySelector(".available-zone").style.height = `${wpPosition - fcPosition}px`;
  
  document.querySelector(".unavailable-zone").style.top = `${wpPosition}px`;
  document.querySelector(".unavailable-zone").style.height = `${containerHeight - wpPosition}px`;
  ...
  if (textureClass.includes("sand") && !textureClass.includes("loam")) {
    soilTip.textContent = "Sandy soils drain quickly and hold less plant-available water. Consider more frequent irrigation.";
  } else if (textureClass.includes("clay")) {
    soilTip.textContent = "Clay soils hold more total water but may have less plant-available water. Avoid overwatering.";
  } else if (textureClass.includes("silt") || textureClass === "loam") {
    soilTip.textContent = "Loamy soils typically have the most plant-available water, providing good growing conditions.";
  } else {
    soilTip.textContent = "Balance between sand, silt, and clay affects how much water is available to plants.";
  }
  
  // Get crop recommendations
  const recommendations = getCropRecommendations(
    waterCharacteristics.textureClass, 
    parseFloat(waterCharacteristics.plantAvailableWater), 
    om
  );
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  document.getElementById("density-input").addEventListener("input", function() {
    const clayValue = parseFloat(document.getElementById("clay-input").value);
    const sandValue = parseFloat(document.getElementById("sand-input").value);
    const siltValue = 100 - clayValue - sandValue;
    const om = parseFloat(document.getElementById("om-input").value);
  ...
  updateWaterCharacteristics(initialClay, initialSand, initialOM, initialDensity);
});
